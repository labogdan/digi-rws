...
Shopify.theme.jsProduct = {
  ...
  enableProductSwatches: function enableProductSwatches() {
    $('body').on('change', '.swatch :radio', function () {
      ...
      var result = $('.product_form, .shopify-product-form').data().product.variants.filter(obj => {
        return obj.title === optionValue
      })

      $("#variant_price").html("<i>" + Shopify.formatMoney(result[0].price, "${{amount}}") + "</i>");
